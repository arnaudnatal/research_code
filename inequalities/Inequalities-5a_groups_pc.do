...
rename assets_total_pc assets_m
...
, title("Wealth per capita") ytitle("1k rupees") ylabel(0(300)1500) ///
xtitle("") ///
legend(order(1 "Dalits" 3 "Middle castes" 5 "Upper castes") pos(6) col(3)) ///
...
graph export "graph_pc/Level_caste_pc.png", as(png) replace
...
graph export "graph_pc/Quintinc_caste_pc.png", as(png) replace
...
foreach i in 2010 2016 2020 {
xtile q_ass_`i'=assets_total_pc if year==`i', n(5)
}
...
title("2010") ///
legend(order(1 "Dalits" 2 "Middle castes" 3 "Upper castes") pos(6) col(3)) ///
note("Pearson Chi2(8)=60.30   Pr=0.00", size(small)) ///
name(compo1, replace)
...
title("2016-17") ///
legend(order(1 "Dalits" 2 "Middle castes" 3 "Upper castes") pos(6) col(3)) ///
note("Pearson Chi2(8)=70.94   Pr=0.00", size(small)) ///
name(compo2, replace)
...
title("2020-21") ///
legend(order(1 "Dalits" 2 "Middle castes" 3 "Upper castes") pos(6) col(3)) ///
note("Pearson Chi2(8)=60.76   Pr=0.00", size(small)) ///
name(compo3, replace)

grc1leg compo1 compo2 compo3, col(3) name(comp, replace) note("{it:Note:} For 405 households in 2010, 492 in 2016-17, and 625 in 2020-21.", size(vsmall))
graph export "graph_pc/Quintass_caste_pc.png", as(png) replace
...
graph export "graph_pc/Source_caste_inc.png", as(png)
...
graph export "graph_pc/Source_caste_ass.png", as(png)
...
cls
* 2010
ineqdeco assets_total_pc if year==2010, by(caste)

cls
* 2016
ineqdeco assets_total_pc if year==2016, by(caste)

cls
* 2020
ineqdeco assets_total_pc if year==2020, by(caste)
...
********** Graph
import excel "Pop_shares.xlsx", sheet("pc") firstrow clear
drop if year==.
...
legend(order(1 "Share in the population" 2 "Share in the total monthly income per capita" 3 "Share in the total wealth per capita") col(1) pos(6)) scale(1.2) note("{it:Note:} For 405 households in 2010, 492 in 2016-17, and 625 in 2020-21.", size(vsmall))
graph export "graph_pc/Shares_pc.png", as(png) replace

...
#--- Matrices creation
X_loan1<-as.matrix(cbind(loanamount1,loanamount2,loanamount3))
X_loan2<-as.matrix(cbind(cro_loanamount1,cro_loanamount2,cro_loanamount3))
X_loan3<-as.matrix(cbind(ihs_loanamount1,ihs_loanamount2,ihs_loanamount3))

#X_income<-as.matrix(cbind(income1,income2,income3))
#X_assets<-as.matrix(cbind(assets1,assets2,assets3))
X_DSR1<-as.matrix(cbind(DSR1,DSR2,DSR3))
X_DSR2<-as.matrix(cbind(cro_DSR1,cro_DSR2,cro_DSR3))
X_DSR3<-as.matrix(cbind(ihs_DSR_1001,ihs_DSR_1002,ihs_DSR_1003))
X_DSR4<-as.matrix(cbind(ihs_DSR_10001,ihs_DSR_10002,ihs_DSR_10003))
...
interactive_clustering(X_loan1)
interactive_clustering(X_loan2)
interactive_clustering(X_loan3)
#interactive_clustering(X_income)
#interactive_clustering(X_assets)
interactive_clustering(X_DSR1)
interactive_clustering(X_DSR2)
interactive_clustering(X_DSR3)
interactive_clustering(X_DSR4)

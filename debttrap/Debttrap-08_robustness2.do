...
********** Tables
esttab r1 r2 r3 r4 using "Household_trim.csv", replace ///
...
********** Land x Gender
reg diff_w5_dsr ///
c.w5_dsr1##c.w5_dsr1##c.w5_dsr1##c.w5_dsr1##i.ownland##i.female ///
age age2 nonmarried ///
occ1 occ2 occ4 occ5 occ6 occ7 ///
educ2 educ3 educ4 log_saving goldquantity ///
HHsize HH_count_child log_wealthbis log_income ///
...
********** Tables
esttab r1 r2 r3 r4 using "Individual_trim.csv", replace ///
	b(3) p(3) eqlabels(none) alignment(S) ///
	drop(_cons) ///
	star(* 0.10 ** 0.05 *** 0.01) ///
	cells("b(fmt(2)star)" "se(fmt(2)par)") ///

...
Using instrumental variable method\footnote{They instruments conscientiousness and emotional stability with shock during childhood \dev{à vérifier}.} on Dutch dataset, they shows that people in the bottom quintile of personality traits are 10 times more likely to experience financial distress than those in the top quintile.
...
To avoid a bias in factor analysis, we do not corrected for acquiescence bias. % because it suppose that one reverse questions have the same meaning as one question.
...
To analyse the relationship between personality traits and individual indebtedness, we proceed in two stages in order to better understand the relationship.

\subsubsection{Role of skills on future debt}

In a first step, we use the lagged personality traits on individual debt (the role of personnality traits in $t$ on the individual debt in $t+1$).
It allow us to understand 
...
To overcome this sample selection issue, we use a \cite{Heckman1976} model to estimate the effect of PT\&CS on indebtedness.
The household debt dependency ratios, defined as the number of indebtedness individuals divided by the total number of household members, in 2016-17 are used as exclusion restriction variables, allowing to compute an Inverse Mill’s Ratios to correct for selection in our equations of interest.
...
In a second step, we fully accept the non-stability of personnality traits by using the variation of Big-5 personality traits through time with fixed effect model. 


Non corrected traits because, de facon globale, meilleure internal consistancy
...
\begin{figure}[ht]
\raggedright
\includegraphics[width=\textwidth]{INPUT/Kernel_efa}
\caption{Distribution of personality traits (from exploratory factor analysis) -- The resulting personality trait are based on the standardised residual from univariate OLS regression with age as exogenous variable. This is the personality trait purged from life-cycle effects}
...
\begin{figure}[ht]
\raggedright
\includegraphics[width=\textwidth]{INPUT/Kernel_cog}
\caption{Distribution of cognitive skills score}
...
\begin{figure}[ht]
\raggedright
\includegraphics[width=\textwidth]{INPUT/Kernel_b5raw16}
\caption{Distribution of Big-5 personality traits in 2016-17 -- The resulting personality trait are non-corrected from acquiescence bias and are based on the standardised residual from univariate OLS regression with age as exogenous variable. This is the personality trait purged from life-cycle effects}
\sourcefig{NEEMSIS-1 (2016-17); author's calculations.}
\label{fig:efascore}
\end{figure}
...
\begin{figure}[ht]
\raggedright
\includegraphics[width=\textwidth]{INPUT/Kernel_b5raw20}
\caption{Distribution of Big-5 personality traits in 2020-21 -- The resulting personality trait are non-corrected from acquiescence bias and are based on the standardised residual from univariate OLS regression with age as exogenous variable. This is the personality trait purged from life-cycle effects}
...
\begin{figure}[htpb]
\raggedright
\includegraphics[scale=0.75]{INPUT/omega}
...
\begin{figure}[htpb]
\raggedright
\includegraphics[width=\textwidth]{INPUT/deltacont_raw}
...
\begin{figure}[htpb]
\raggedright
\includegraphics[width=\textwidth]{INPUT/deltacont_cor}
...
\begin{figure}[htpb]
\raggedright
\includegraphics[width=\textwidth, angle=0]{INPUT/factor2016_Raw}
\caption{Results on factor of exploratory factor analysis}
\sourcefig{NEEMSIS-1 (2016-17); author's calculations.}
\label{fig:stab}
\end{figure}

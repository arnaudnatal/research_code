...
In what follows, we use literature on \PTCS~ and indebtedness to further document and complement the analyses. 
...
On the shaping of debt, only Raven test is correlated (positively) with the total amount of debt for dalit female, \aebe.
...
Looking at personality traits, we have first identifies the crucial role of OP-EX and CO in shaping debt especially for non-dalit male, and dalit female.
As shown in the literature \citep{Brown2014}, OP-EX is positively correlated with the probability of holding debt while our results suggest negative relationship for non-dalits female.
We also find positive correlation with the total loan amount and the individual debt service ratio respectively for non-dalit male and dalit female, which coroborate with \cite{Brown2014}.
Using panel data variation, it appear that extraversion is negatively correlated with the amount of debt.
FIndings on Openness and Extraversion, thus \dev{suggest}
...
% Concerning cognitive skills, \citep{Agarwal2013}.





% \paragraph{Contextualization of the findings}

% Ici parler des quantiles regressions pour dire que le long de la distribution, certains traits jouent plus ou moins
...
CS are less good predictor than PT.
It is correlated only with the amount of debt au premier tier (-) et au dixième (+)


Les CS ne shape pas la dette.
C'est OPEX qui shape positivement la dette (le montant et le fardeau)
CO shape aussi, mais en negatively.
